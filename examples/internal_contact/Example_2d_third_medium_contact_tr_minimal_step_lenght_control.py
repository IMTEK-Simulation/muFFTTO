...
are measured and printed every increment -- `mean imposed F_10` against the
...
def admissible_scale_field(F_in, dF_in):
    """Per-quadrature-point largest s > 0 keeping det(F + s*dF) > 0.

    Returns the LOCAL field, unreduced: entry (q, x, y) is how far that one
    point can travel along dF before its own det F reaches zero, +inf if it
...
def apply_throttled_load(dlam):
    ...
    """
    s_qxy = admissible_scale_field(total_strain_field,
                                   macro_gradient_full_field)

    with np.errstate(divide='ignore', invalid='ignore'):
        theta = np.minimum(1.0, PIXEL_SAFETY * s_qxy / dlam)
...
for inc in range(1, ninc + 1):
    lam_nominal = inc / float(ninc)

    # ---- throttled load step ----------------------------------------------
    # F at the converged previous state sets each point's own room to move.
    _set_F_from(u)
    n_throttled, theta_min = apply_throttled_load(dlam_nominal)
    ...
    F10_target = lam_nominal * H_macro[1, 0]
    F10_actual = global_mean(imposed_field.s[1, 0])

    root_print('-' * 70)
    root_print(f'increment {inc:4d}   lam_nominal = {lam_nominal:.4f}')
    root_print(f'  throttled points {n_throttled:5d}   theta_min '
               f'{theta_min:8.4f}   min J after load {J_pred:10.3e}')
    root_print(f'  imposed F_10: target {F10_target:+.5f}  '
               f'actual (mean) {F10_actual:+.5f}  '
               f'drift {F10_actual - F10_target:+.3e}')
    ...
    hist_F10.append(F10_target)
    hist_F10_actual.append(F10_actual)
    ...
    if serial and plot_every and inc % plot_every == 0:
        ...
        visualization_utils.plot_field_on_grid(
            ...
            name=f'phase, increment {inc}   lam_nom = {lam_nominal:.4f}')

root_print('=' * 70)
root_print(f'final min J : {min_J(total_strain_field):.6e}')
root_print(f'imposed F_10: target {hist_F10[-1]:+.5f}   '
           f'actual (mean) {hist_F10_actual[-1]:+.5f}')
...
if rank == 0 and len(hist_lam) > 1:
    ...
    ax[0].plot(hist_F10_actual, hist_P10, '-o', ms=3, color='k',
               label='actual (mean)')
    ax[0].plot(hist_F10, hist_P10, '--', color='0.6', label='nominal')
    ax[0].set_xlabel(r'$\bar{F}_{10}$')
    ax[0].set_ylabel(r'$\bar{P}_{10}$')
    ax[0].legend(fontsize=8)

    ax[1].plot(hist_lam, np.asarray(hist_F10_actual) - np.asarray(hist_F10),
               '-o', ms=3, color='C0')
    ax[1].set_xlabel(r'$\lambda_{\rm nominal}$')
    ax[1].set_ylabel(r'imposed $F_{10}$ drift')

    ax[2].semilogy(hist_lam, hist_minJ, '-o', ms=3, color='C3')
    ax[2].set_xlabel(r'$\lambda_{\rm nominal}$')
    ax[2].set_ylabel(r'$\min \det F$')

    ax[3].plot(hist_lam, hist_throttled, '-o', ms=3, color='C2')
    ax[3].set_xlabel(r'$\lambda_{\rm nominal}$')
