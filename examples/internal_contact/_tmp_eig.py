...
# ---- throwaway diagnostic: smallest eigenvalue of hess Pi at the end state --
from scipy.sparse.linalg import LinearOperator, eigsh

n_loc = int(np.prod(u_shape))
lam_current = lam
_tangent_x = None
hessp(u, np.zeros(u_shape))          # prime the tangent cache at the end state

def _mv(w):
    return hessp(u, w.reshape(u_shape)).ravel()

B = LinearOperator((n_loc, n_loc), matvec=_mv, dtype=float)
ev_min = eigsh(B, k=4, which='SA', tol=1e-6, maxiter=20000,
               return_eigenvectors=False)
ev_max = eigsh(B, k=1, which='LA', tol=1e-3, return_eigenvectors=False)
print('=' * 70)
print(f'Hessian spectrum at lam = {lam:.6f}')
print(f'  4 smallest eigenvalues : {np.sort(ev_min)}')
print(f'  largest eigenvalue     : {ev_max[0]:.6e}')
print(f'  INDEFINITE             : {bool(np.min(ev_min) < 0.0)}')
