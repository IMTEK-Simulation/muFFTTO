...
preconditioner_fnfnqks = discretization.get_preconditioner_Green_mugrid(reference_material_data_ijkl=conductivity_C_0)
...
nb_load_cases = 2
macro_gradients = np.zeros([nb_load_cases, dim])
macro_gradients[0] = np.array([1.0, 0.0])
macro_gradients[1] = np.array( [0.0, 1.0])
...
if MPI.COMM_WORLD.rank == 0:
    print(f'Target conductivity tensor:\n{conductivity_C_target}')

# Target fluxes
target_fluxes = np.zeros([nb_load_cases, dim])

for load_case in range(nb_load_cases):
    target_fluxes[load_case] = np.einsum('ij,j->i', conductivity_C_target, macro_gradients[load_case])

    if MPI.COMM_WORLD.rank == 0:
        print(f'Load case {load_case}: target flux = {target_fluxes[load_case].tolist()}')

# Optimization state variables
temperature_field_load_case = [discretization.get_temperature_sized_field(name=f'u_{i}') for i in range(nb_load_cases)]
adjoint_field_load_case = [discretization.get_temperature_sized_field(name=f'adj_{i}') for i in range(nb_load_cases)]
...
material_data_field_C_0_rho_ijqxyz = discretization.get_material_data_size_field_mugrid(name='mat_data_field')
...
def objective_function_multiple_load_cases(phase_field_1nxyz_flat):
    ...
    material_data_field_C_0_rho_ijqxyz.s[...] = (conductivity_C_0 - conductivity_C_void)[
                                                      ..., np.newaxis, np.newaxis, np.newaxis] * \
                                                  np.power(phase_field_at_quad_poits_1qxyz.s, p)[0, 0, :, ...] + \
                                                  conductivity_C_void[..., np.newaxis, np.newaxis, np.newaxis]
    ...
    def K_fun(x, Ax):
        discretization.apply_system_matrix_mugrid(material_data_field=material_data_field_C_0_rho_ijqxyz,
                                                  input_field_inxyz=x,
    ...
    for load_case in np.arange(nb_load_cases):
        ...
        solvers.conjugate_gradients_mugrid(
            ...
            x=temperature_field_load_case[load_case],
            P=M_fun,
            tol=cg_setup['cg_tol'],
            maxiter=10000,
            callback=callback)
        ...
        f_sigmas[load_case] = topology_optimization.compute_flux_equivalence_potential(
            actual_flux_ij=homogenized_fluxes[load_case],
            target_flux_ij=target_fluxes[load_case])

        s_flux_and_adjoint_load_case.s[0, 0], adjoint_field_load_case[
            load_case], adjoint_energies[
            load_case], info_adjoint_current = topology_optimization.sensitivity_flux_and_adjoint(
            discretization=discretization,
...
if __name__ == '__main__':
    ...
    material_data_field_C_0_rho_quad.s[...] = (conductivity_C_0 - conductivity_C_void)[..., np.newaxis, np.newaxis, np.newaxis] * \
    ...
    homogenized_C_ijkl = np.zeros(np.array(2 * [dim, ]))
    ...
    for i in range(2):
        macro_gradient_ij = np.zeros([dim ])
        macro_gradient_ij[i ] = 1
    ...
    if MPI.COMM_WORLD.rank == 0:
        print('Optimized elastic tangent =  :\n' +
              np.array2string( homogenized_C_ijkl,
