...
class TestBiquadraticQuadElement:
    ...
    def test_shapes(self, q9_element):
        ...
        assert elem.N_at_quad_points_dqnijk.shape == (1, 9, 4, 2, 2)
        # B: (dim, n_qp, 1, 3, 3)
    ...
    def test_kronecker_delta_property(self, q9_element):
        ...
        nodes_1d = [-1.0, -0.5, 0.0, 0.5, 1.0]
        ref_nodes = np.array([[xi, eta] for eta in nodes_1d for xi in nodes_1d])  # (9, 2)

        for j, node_coord in enumerate(ref_nodes):
            eval_matrix = np.array(elem.shape_functions(node_coord))

            assert np.allclose(np.sum(eval_matrix), 1, atol=1e-15), \
                f"Kronecker delta property failed: max error = {np.max(np.abs(np.sum(eval_matrix) - 1))}"

    def test_partition_of_unity(self, q9_element):
        """Test sum of N over nodes is 1, and sum of derivatives over nodes is 0."""
        elem = q9_element

        # N sums to 1 over node axes at all quad points # locked for 2D
        sum_N = np.sum(elem.N_at_quad_points_dqnijk, axis=(-3, -2, -1))  # (1, 9, 1)
        ...
        sum_B = np.sum(elem.B_grad_at_pixel_dqnijk, axis=(-3, -2, -1))  # (2, 9, 1)
        ...
        sum_H = np.sum(elem.H_hess_at_pixel_deqnijk, axis=(-3, -2, -1))  # (2, 2, 9, 1)
    ...
    def test_exact_recovery_of_full_biquadratic_field(self, q9_element, non_square_pixel_size):
        ...
        u_nodes = u_poly(X, Y)
        u_nodes_nij = np.zeros(elem.node_layout)
        u_nodes_nij[0, 0, 0] = u_nodes[0, 0]
        u_nodes_nij[1, 0, 0] = u_nodes[1, 0]
        u_nodes_nij[2, 0, 0] = u_nodes[0, 1]
        u_nodes_nij[3, 0, 0] = u_nodes[1, 1]

        u_nodes_nij[0, 1, 0] = u_nodes[2, 0]
        u_nodes_nij[1, 1, 0] = 0
        u_nodes_nij[2, 1, 0] =u_nodes[2, 1]
        u_nodes_nij[3, 1, 0] = 0

        u_nodes_nij[0, 0, 1] = u_nodes[0, 2]
        u_nodes_nij[1, 0, 1] = u_nodes[1, 2]
        u_nodes_nij[2, 0, 1] = 0
        u_nodes_nij[3, 0, 1] = 0

        u_nodes_nij[0, 1, 1] = u_nodes[2, 2]
        u_nodes_nij[1, 1, 1] = 0
        u_nodes_nij[2, 1, 1] = 0
        u_nodes_nij[3, 1, 1] = 0
        ...
        u_interp = np.einsum('aqnij,nij->aq', elem.N_at_quad_points_dqnijk, u_nodes_nij)
        ...
        grad_u_interp = np.einsum('dqnij,nij->dq', elem.B_grad_at_pixel_dqnijk, u_nodes_nij)
        ...
        hess_u_interp = np.einsum('deqnij,nij->deq', elem.H_hess_at_pixel_deqnijk, u_nodes_nij)
    ...
    def test_element_factory_registration(self, non_square_pixel_size):
        ...
        assert element.node_layout == ( 4, 2, 2)
        assert element.B_grad_at_pixel_dqnijk.shape == (2, 9, 4, 2, 2)
