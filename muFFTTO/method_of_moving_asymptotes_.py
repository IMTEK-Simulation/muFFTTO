...
def mma_loop(x0, f, gradf, atol=1e-6, **parameters):  # (func_0,x_0,)

    n = parameters['nb_unknown']
    # Initial settings
    m = parameters['nb_constrains']  # just one constrain ---- one for every varieble

    eeen = np.ones((n, 1))  # Ones -  number of unknowns
    eeem = np.ones((m, 1))  # Ones - number of constraints
    zeron = np.zeros((n, 1))  # Zeros -  number of unknowns
    zerom = np.zeros((m, 1))  # Zeros -  number of unknowns

    xval = x0.reshape(n, 1)  # Current values of the design variables.
    xold1 = xval.copy()
    xold2 = xval.copy()

    xmin = parameters['xmin']  # : Minimum bounds for the design variables.
    xmax = parameters['xmax']  # : Maximum bounds for the design variables.

    low = np.copy(xmin)  #: Lower bounds for the design variables in the constraints.
    upp = np.copy(xmax)  #: Upper bounds for the design variables in the constraints.
    ...
    innerit = 0
    outeriter = 0
    maxoutit = parameters['maxoutit']
    kkttol = 0

    # Calculate function values and gradients of the objective and constraints functions

    # f0val, df0dx, fval, dfdx = f(xval, n, eeen, zeron)
    # objective function at the x_0
    f0val = f(xval)
    df0dx = gradf(xval)  # .reshape(n,1)
    # constrain functions at the x_0 -----> Not BOUNDS!!!
    fval = np.zeros(1)
    dfdx = np.zeros([m, n])
    ...
    norm_dfdx = np.dot(df0dx[:, 0], df0dx[:, 0])
    norm_delx = 1
    # while kktnorm > kkttol and outit < maxoutit:
    while norm_dfdx > grad_norm and norm_delx > norm_delx_tol and outit < maxoutit:
        outit += 1
        outeriter += 1

        # The MMA subproblem is solved at the point xval:
        xmma, ymma, zmma, lam, xsi, eta, mu, zet, s, low, upp = mmasub(m=m, n=n, iter=outeriter, xval=xval,
                                                                       xmin=xmin, xmax=xmax,
                                                                       xold1=xold1, xold2=xold2, f0val=f0val,
                                                                       df0dx=df0dx, fval=fval, dfdx=dfdx,
                                                                       low=low, upp=upp, a0=a0, a=a, c=c, d=d,
                                                                       move=move,
                                                                       asyinit=0.5, asydecr=0.7, asyincr=1.2,
                                                                       asymin=0.01, asymax=10, raa0=0.00001,
                                                                       albefa=0.1)
        if outit % 10 ==0:
            plt.figure()
            shape = np.sqrt(xmma.size).astype(int)
            field = xmma.reshape([shape, shape])
            plt.contourf(field)
            # nodal_coordinates[0, 0] * number_of_pixels[0], nodal_coordinates[1, 0] * number_of_pixels[0],
            plt.clim(0, 1)
            plt.colorbar()
            plt.title('Iteration = %.0f' % (outit))
            plt.show()
        # # The MMA subproblem is solved at the point xval:
        # results = mma_subproblem_ML2025(m=m, n=n, iter=outeriter,
        #                                 xval=xval,
        #                                 xmin=xmin, xmax=xmax,
        #                                 xold1=xold1, xold2=xold2,
        #                                 f0val=f0val,
        #                                 df0dx=df0dx, fval=fval, dfdx=dfdx,
        #                                 low=low, upp=upp, a0=a0, a=a, c=c,
        #                                 d=d,
        #                                 move=move,
        #                                 asyinit=0.5, asydecr=0.7,
        #                                 asyincr=1.2,
        #                                 asymin=0.01, asymax=10,
        #                                 raa0=0.00001,
        #                                 albefa=0.1)
        ...
        # f0val, df0dx, fval, dfdx = func_0(xval, n, eeen, zeron)
        # objective function at the x_0
        f0val = f(xval)
        df0dx = gradf(xval)  # .reshape(n,1)
        # constrain functions at the x_0 -----> Not BOUNDS!!!
        ...
        norm_dfdx = np.dot(df0dx[:, 0], df0dx[:, 0])
        print(f'norm_dfdx = {norm_dfdx}')
        norm_delx = np.dot(xval[:, 0] - xold1[:, 0], xval[:, 0] - xold1[:, 0])

        print(f'x_(i) - x_(i-1) = {norm_delx}')

        print(f0val)

    x_solution = xval
    return x_solution, f0val, outit,
...
def mmasub(m: int, n: int, iter: int, xval: np.ndarray, xmin: np.ndarray, xmax: np.ndarray,
           ...
           d: np.ndarray, move: float, asyinit: float, asydecr: float, asyincr: float,
           asymin: float, asymax: float, raa0: float, albefa: float):
    """
    ...
    if iter <= 2:
        ...
        factor = eeen.copy()  # TODO{Names} factor  should be gamma
    ...
    # EQ: 3.3-4 right constant
    xmami = xmax - xmin
    xmami_eps = 0.00001 * eeen
    xmami = np.maximum(xmami, xmami_eps)
    xmami_inv = eeen / xmami
    # EQ: 3.3
    ux1 = upp - xval  ## upper asymptote: u - x_
    ux2 = ux1 * ux1  # quadratic term
    # EQ: 3.4
    xl1 = xval - low  # lower asymptote: x_ - l
    xl2 = xl1 * xl1  # quadratic term
    # EQ: 3.5
    ux_inv = eeen / ux1
    xl_inv = eeen / xl1
    ...
    p0 = zeron.copy()  #
    q0 = zeron.copy()
    # largest and smollest gradient
    p0 = np.maximum(df0dx, 0)
    q0 = np.maximum(-df0dx, 0)

    # I do not know why is 0.001. I do not get where he took it
    # pq0 = 0.001 * (p0 + q0) + raa0 * xmami_inv
    # p0 = p0 + pq0
    # p0 = p0 * ux2
    # EQ 3.3
    p0 = ux2 * (1.001 * p0 + 0.001 * q0 + raa0 * xmami_inv)

    # q0 = q0 * xl2
    # q0 = q0 + pq0
    # EQ 3.3
    q0 = xl2 * (0.001 * p0 + 1.001 * q0 + raa0 * xmami_inv)

    # This part is for Inequality constraints
...
def gcmmasub(m: int, n: int, iter: int, epsimin: float, xval: np.ndarray, xmin: np.ndarray,
             ...
             a: np.ndarray, c: np.ndarray, d: np.ndarray, albefa: float):
    """
    ...
    # Value of the approximation function at the point
...
def mma_subproblem_ML2025(m: int, n: int, iter: int, xval: np.ndarray, xmin: np.ndarray, xmax: np.ndarray,
                          xold1: np.ndarray, xold2: np.ndarray, f0val: float, df0dx: np.ndarray, fval: np.ndarray,
                          dfdx: np.ndarray, low: np.ndarray, upp: np.ndarray, a0: float, a: np.ndarray, c: np.ndarray,
                          d: np.ndarray, move: float, asyinit: float, asydecr: float, asyincr: float,
                          asymin: float, asymax: float, raa0: float, albefa: float):
    ...
    # b = np.dot(P, ux_inv) + np.dot(Q, xl_inv) - fval
    def subproblem(xxx):
        # this function is subproblem
        #
        # f(x)= f(x_k)+sum(p/{upper-x}+p/{x-lower}) - sum(p/{upper-x_k}+p/{x_k-lower})
        #
        u_x = upp - xxx
        l_x = xxx - low
        # eeen / ux1
        f_x = fval + np.dot(p0[:, 0], 1 / u_x[:, 0]) + np.dot(q0[:, 0], 1 / l_x[:, 0]) - (
                np.dot(p0[:, 0], 1 / ux1[:, 0]) + np.dot(q0[:, 0], 1 / xl1[:, 0]))
        return f_x

    # Solving the subproblem using the primal-dual Newton method

    bounds = sp.optimize.Bounds(lb=xmin, ub=xmax, keep_feasible=True)

    fxss = subproblem(xval)
    xopt = sp.optimize.minimize(subproblem,
                                xval[:, 0],
                                options={'gtol': 1e-6,
                                         'disp': True})
...
def raaupdate(xmma: np.ndarray, xval: np.ndarray, xmin: np.ndarray, xmax: np.ndarray, low: np.ndarray, upp: np.ndarray,
              ...
              raa: np.ndarray, raa0eps: np.ndarray, raaeps: np.ndarray, epsimin: float) -> Tuple[
...
def concheck(m: int, epsimin: np.ndarray, f0app: np.ndarray, f0valnew: np.ndarray, fapp: np.ndarray,
             fvalnew: np.ndarray) -> int:
    """
...
def asymp(outeriter: int, n: int, xval: np.ndarray, xold1: np.ndarray, xold2: np.ndarray, xmin: np.ndarray,
          ...
          asyincr: float = 1.2, asymin: float = 0.01, asymax: float = 10) -> Tuple[
