...
conductivity_C_0 = np.array([[1., 0],
                             [0, 1.0]])
conductivity_C_1 = np.array([[2., 0],
                             [0, 2.0]])
conductivity_C_2 = np.array([[3., 0], [0, 3.0]])

material_data_field_C_global = discretization.get_material_data_size_field_mugrid(name='conductivity_tensor')
material_data_field_C_global.s.fill(0)

# populate the field with zeros material
...
# artificial adding of third phase
phase_indicator_array[5:10,7:16]=2

phase_field = discretization.get_scalar_field(name='phase_field')
phase_field.s[0, 0] = phase_indicator_array
# here you create masks for individual phases
mat_0_mask= phase_indicator_array == 0
mat_1_mask= phase_indicator_array == 1
mat_2_mask= phase_indicator_array == 2

# apply material distribution  - assign specific local (d,d) conductivity matrix to the global data field
material_data_field_C_global.s[..., mat_0_mask] =  conductivity_C_0[...,np.newaxis,np.newaxis]
material_data_field_C_global.s[..., mat_1_mask] =  conductivity_C_1[...,np.newaxis,np.newaxis]
material_data_field_C_global.s[..., mat_2_mask] =  conductivity_C_2[...,np.newaxis,np.newaxis]
# --------------------------------------------------------------------------------------------------------------------- #
...
def K_fun(x, Ax):
    ...
    discretization.apply_system_matrix_mugrid_deformed_grid(material_data_field=material_data_field_C_global,
                                                            input_field_inxyz=x,
...
for i in range(dim):
    ...
    # Solve equilibrium
    rhs_field.sg.fill(0)
    discretization.get_rhs_mugrid_deformed_grid(material_data_field_ijklqxyz=material_data_field_C_global,
                                                macro_gradient_field_ijqxyz=macro_gradient_field,
                                                rhs_inxyz=rhs_field,
                                                det_of_deformation_gradient=det_F,
                                                inv_of_deformation_gradient=inv_F)
    ...
    homogenized_A_ij[i, :] = discretization.get_homogenized_stress_mugrid_deformed_grid(
        material_data_field_ijklqxyz=material_data_field_C_global,
