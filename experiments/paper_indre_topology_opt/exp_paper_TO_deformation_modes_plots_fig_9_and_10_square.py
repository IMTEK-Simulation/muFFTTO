...
parser.add_argument('--poison_target_1', type=float, default=-0.5, help='Poison target value')
parser.add_argument('--poison_target_2', type=float, default=-0.3, help='Poison target value')

parser.add_argument('--weight', type=float, default=20.0, help='Weight value')
parser.add_argument('--grid_type', type=str, default='square', choices=['hexa', 'square'], help='Grid type')
...
poison_target_1 = args.poison_target_1
poison_target_2 = args.poison_target_2

weight = args.weight

experiment = 5

script_name = f'exp_paper_TO_exp_{experiment}_{grid_type}' + f'_random_{random_init}' + f'_N_{N}' + f'_cgtol_{cg_tol_exponent}' + f'_soft_{soft_phase_exponent}'
...
if plot_deformation_with_insets:
    _index_multiplier = 0
    for poison_target in [poison_target_1, poison_target_2]:  # poison_target_2
        ...
        #ax_global.text(0, 1.1, rf'$\mathbf{{({letter})}}$', transform=ax.transAxes)
        ...
        for plot_idx, load_increment in enumerate(load_increments_to_plot):
            ...
            if grid_type == 'hexa':
                ...
                letter_offset = -0.1
            #letter = string.ascii_lowercase[plot_idx+_index_multiplier*4]
            #ax.text(letter_offset, 1.1, rf'$\mathbf{{({letter})}}$', transform=ax.transAxes)

            if poison_target == poison_target_1:
                letter = 'a'
            else:
                if grid_type == 'hexa':
                    letter = 'g'
            ...
            ax.text(letter_offset, 1.05, rf'$\mathbf{{({letter}.{plot_idx})}}$', transform=ax.transAxes)
            ...
            if point_hinge is not None:
                ...
                if plot_idx == 0 and grid_type == 'hexa':
                ...
                if grid_type == 'square' and weight == 20 and poison_target == -0.5:
                    ...
                    axins.plot(point_hinge[0], point_hinge[1] + 0.35, 'ro', linestyle='--', mew=1, ms=25,
                               fillstyle='none')
                ...
                if grid_type == 'square' and weight == 10 and poison_target == -0.5:
                    # Plot the circle around the hinge point
                    axins.plot(point_hinge[0], point_hinge[1] + 0.33, 'ro', linestyle='--', mew=1, ms=25,
                               fillstyle='none')
                ...
                axins.pcolormesh(x_coords[0], x_coords[1], tilled_phase_masked,
                                 # transpose because tilled phase masked is transpose.. no power to search why
                                 shading='flat',
                                 edgecolors='none',
                                 lw=0.01,
                                 cmap=cmap_custom,
                ...
                if poison_target == poison_target_1:
                    ...
                    if grid_type == 'hexa':
                        ...
                        letter = 'C'
                #letter_bottom = string.ascii_uppercase[plot_idx+_index_multiplier*4]
                #axins.text(-0.15, 0.85, rf'$\mathbf{{{letter_bottom}}}$', transform=axins.transAxes)
                axins.text(-0.30, 0.85, rf'$\mathbf{{{letter}.{plot_idx}}}$', transform=axins.transAxes)
                ...
                del x_ref, x_coords, lin_disp_ixy, tilled_disp_x, tilled_disp_y, displacement
                gc.collect()
