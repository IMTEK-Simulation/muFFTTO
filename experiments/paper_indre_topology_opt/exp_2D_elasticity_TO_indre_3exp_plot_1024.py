...
# get nodal points positions
x_coords = np.zeros([2, 3*N,3*N])
x = np.linspace(0, 3*Lx, 3*N)
y = np.linspace(0, 3*Ly, 3*N)
x_coords[0], x_coords[1] = np.meshgrid(x, y, indexing='ij')

macro_grad_increment_ij=np.zeros((2, 2))
gamma=0.01
macro_grad_increment_ij=np.array([[gamma,gamma/2],
                                  [gamma/2,gamma]])
# # add linear displacement
imposed_disp_ixy = np.einsum('ij...,j...->i...', macro_grad_increment_ij  , x_coords)
x_coords_def = x_coords + imposed_disp_ixy #+ total_displacement_fluctuation_ixy
#
# x_coords_def = x_coords + imposed_disp_ixy + total_displacement_fluctuation_ixy
fig, ax = plt.subplots(1, 1, figsize=(10, 10))
pcm = ax.pcolormesh(x_coords_def[0], x_coords_def[1], np.tile(phase_field, (3, 3)),
                          # cmap=mpl.cm.Greys, vmin=contrast, vmax=1,
                          linewidth=0,
                          edgecolors='k',
                          rasterized=True)
