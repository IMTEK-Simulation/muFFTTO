...
#    ax[0].set_title(f'Square grid - L-BFGS iteration {iter}')
    ...
    fig, ax = plt.subplots(1, 2, figsize=(5, 4.5), width_ratios=[0.03, 1],
                           constrained_layout=True)
...
#    ax[0].set_title(f'Square grid - L-BFGS iteration {iter}')
    ...
    for a in ax[1:3]:
        ...
        a.set_aspect('equal')
    #fig.tight_layout()
    fname = f'square_grid_auxetic_{iter}' + '{}'.format('.pdf')
...
gs = fig.add_gridspec(1, 1, height_ratios=[1], hspace=0.1)

# Add subplot in the second column
ax_void = fig.add_subplot(gs[0])
# ax = fig.add_subplot(gs[0, 1])
soft_phase_exponents = [5]  # 3,
for e in np.arange(len(soft_phase_exponents)):
    soft_phase_exponent = soft_phase_exponents[e]
...
l5, = ax_void.semilogx(np.array(Ns[:3]) ** 2, nbit_per_lbfgs_adjoint_G[:3], "g-.", marker='^',
                   label=rf'Adjoint problem - Green - $\kappa^{{soft}}  =0$',
                   linewidth=2)
lines_void.append(l5)
labels_void.append(r'Adjoint – Green – $\kappa^{\mathrm{soft}} = 0$')
...
l6, = ax_void.semilogx(np.array(Ns[:3]) ** 2, nbit_per_lbfgs_mech_G[:3], "g-.", marker='x',
                   label=rf'Mechanical equilibrium - Green - $\kappa^{{soft}}  =0$',
                   linewidth=2)
lines_void.append(l6)
labels_void.append(r'Mechanical eq. – Green – $\kappa^{\mathrm{soft}} = 0$')
...
# ax.set_ylim([10, 1e4])
# ax.legend(loc="center left", bbox_to_anchor=(1, 0.5))
fname = figure_folder_path + 'scaling_green_only{}'.format('.pdf')
print(('create figure: {}'.format(fname)))
plt.savefig(fname, bbox_inches='tight')
plt.show()
