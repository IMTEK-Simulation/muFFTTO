...
parser = argparse.ArgumentParser(
    prog=script_name, description="Solve topology  problem for different weights"
)
...
soft_phase_exponent = int(args.soft_phase_exponent)
preconditioner_type = args.preconditioner_type
save_data = args.save_phases
eta = float(args.eta_parameter)
weight = float(args.weight_parameter)
...
def objective_function_multiple_load_cases(phase_field_1nxyz_flat):
    ...
    f_phase_field, f_rho_grad, f_rho = topology_optimization.objective_function_phase_field(discretization=discretization,
                                                                         phase_field_1nxyz=phase_field_1nxyz,
                                                                         eta=eta,
                                                                         double_well_depth=double_well_depth_test,
                                                                         split_results=True)
    norms_pf_grad.append(f_rho_grad)
    norms_pf_rho.append(f_rho)
...
if __name__ == '__main__':
    ...
    script_name = os.path.splitext(os.path.basename(__file__))[
                      0] + f'_random_{random_init}' + f'_N_{number_of_pixels[0]}' + f'_cgtol_{cg_tol_exponent}' + f'_soft_{soft_phase_exponent}'
    file_folder_path = os.path.dirname(os.path.realpath(__file__))  # script directory
    data_folder_path = file_folder_path + '/exp_data/' + script_name + '/'
    figure_folder_path = file_folder_path + '/figures/' + script_name + '/'
    ...
    # Nx, Ny =number_of_pixels
    # phase_field_0.s[0, 0,:Nx //4,:] =1
    # phase_field_0.s[0, 0, 3*Nx // 4:, :] = 1
    # phase_field_0.s[0, 0,:,:Ny //4] =1
    # phase_field_0.s[0, 0, :, 3*Ny // 4:] = 1.
    # interphase_width = 1/Nx
    #
    # plt.figure(    )
    # plt.imshow(phase_field_0.s[0, 0], cmap='jet')
    # plt.show()
    # # objective function phase field terms
    ...
    #                                                                      eta=interphase_width,
    #                                                                      double_well_depth=1)
    # print(f'f_phase_field={f_phase_field}')
    # quit()
    ...
    def my_callback(result_norms):
        global iterat
        file_data_name_it = f'_eta_{eta:.2f}' + f'_w_{weight}' + f'_iteration_{iterat}'  # print('rank' f'{MPI.COMM_WORLD.rank:6} ')
    ...
    _info['norms_pf_grad'] = norms_pf_grad

    _info['norms_pf_rho'] = norms_pf_rho

    _info['norms_adjoint_energy'] = norms_adjoint_energy

    _info['nb_iterations'] = iterat

    file_data_name = f'_eta_{eta:.2f}' + f'_w_{weight}' + f'_final'
    ...
    if MPI.COMM_WORLD.rank == 0:
        np.savez(data_folder_path + f'{preconditioner_type}' + f'_eta_{eta:.2f}' + f'_w_{weight}' + f'_log.npz',
