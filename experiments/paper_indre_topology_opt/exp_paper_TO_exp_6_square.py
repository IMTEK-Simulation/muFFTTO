...
parser = argparse.ArgumentParser(
    prog=script_name, description="Solve topology  problem for continuously increasing weight"
)
...
poison_target = float(args.target_poisson)
...
if __name__ == '__main__':
    ...
    if start == 0:
        ...
        file_data_name = f'_eta_{eta:.2f}'  + f'_p_{poison_target}' + f'_iteration_{start}'  # print('rank' f'{MPI.COMM_WORLD.rank:6} ')
    ...
    def my_callback(result_norms):
        global iterat
        file_data_name_it = f'_eta_{eta}' + f'_p_{poison_target}' + f'_iteration_{iterat}'  # print('rank' f'{MPI.COMM_WORLD.rank:6} ')
    ...
    file_data_name = f'_eta_{eta}' +  f'_p_{poison_target}' + f'_final'
    ...
    if MPI.COMM_WORLD.rank == 0:
        np.savez(data_folder_path + f'{preconditioner_type}' + f'_eta_{eta}'   + f'_p_{poison_target}'+ f'_log.npz',
