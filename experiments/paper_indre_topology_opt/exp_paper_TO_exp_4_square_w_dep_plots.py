...
for i in range(Cij_target.shape[0]):
    for j in range(i, Cij_target.shape[1]):
        ...
        line, = plt.semilogx(weights, C_computed[:, i, j], '-', linewidth=2, marker='|',
                             label=f' - {label_c}  ')
        plt.axhline(y=Cij_target[i, j], color=line.get_color(), linestyle='-.', linewidth=2,
...
plt.xlim(0.1, 100)
plt.ylim(-0.5, 1)
plt.title(r'Square grid : 3 load cases' + f' N={N}, eta={eta_mult}')
fname = figure_folder_path + 'exp4_square_convergence_cc{}'.format('.pdf')
...
plt.title(r'Square grid : 3 load cases' + f' N={N}, eta={eta_mult}')
fname = figure_folder_path + 'exp4_square_convergence_rel_cc{}'.format('.pdf')
print(('create figure: {}'.format(fname)))
plt.savefig(fname, bbox_inches='tight')
plt.show()
...
plt.ylim(-0.5, 2)
plt.title(r'Square grid : 3 load cases' + f' N={N}, eta={eta_mult}')
fname = figure_folder_path + 'exp4_square_convergence{}'.format('.pdf')
print(('create figure: {}'.format(fname)))
plt.savefig(fname, bbox_inches='tight')
# plt.show()
plt.figure()
...
for upper_ax in np.arange(5):
    weight = np.array([weights[0], weights[1], weights[4], weights[6], weights[-2]])[upper_ax]
    ...
    if upper_ax == 0:
        ...
        ax1 = fig.add_axes([0.74, 0.46, 0.18, 0.18], transform=ax5.transAxes)
        roll_x = 0
        roll_y = 0
        ax5.annotate('',
                     xy=(weight, f_sigmas[np.where(weights == weight)[0][0]]),
...
fname = figure_folder_path + 'exp4_square{}'.format('.pdf')
print(('create figure: {}'.format(fname)))
plt.savefig(fname, bbox_inches='tight')
# plt.show()
