...
#
# random = False
# script_name = f'exp_paper_JG_2D_elasticity_TO_load_init_random_{random}' + f'_N_{N}/'
# preconditioner_type = 'Green_Jacobi'
# file_name = f'_log.npz'
# _info_log_G_F = np.load(
#     './exp_data/' + script_name + f'{preconditioner_type}'  + file_name,
#     allow_pickle=True)
# phase_field_it_F = np.load('./exp_data/' + script_name + f'{preconditioner_type}' + f'_iteration_{_info_log_G_F.f.nb_iterations}' + '.npy',
#                          allow_pickle=True)
#
# random = True
# script_name = f'exp_paper_JG_2D_elasticity_TO_load_init_random_{random}' + f'_N_{N}/'
# _info_log_G_T = np.load(
#     './exp_data/' + script_name + f'{preconditioner_type}'  + file_name,
#     allow_pickle=True)
# phase_field_it_T = np.load('./exp_data/' + script_name + f'{preconditioner_type}' + f'_iteration_{_info_log_G_T.f.nb_iterations}' + '.npy',
#                          allow_pickle=True)
#
...
steps = np.arange(0, 500, 11)
Ns = [16, 32, 64, 128, 256, 512, 1024]  # , 64, 128
...
for j in np.arange(len(Ns)):
    ...
    try:
        script_name = f'exp_paper_JG_2D_elasticity_TO_load_init_random_{random}' + f'_N_{N}/'
        _info_log_GJ = np.load('./exp_data/' + script_name + f'{preconditioner_type}' + file_name, allow_pickle=True)
...
ax.plot(np.array(Ns[:]) ** 2, nbit_per_lbfgs_adjoint_G, "g-.", marker='x',
        label=r'Adjoint problem - Green ',
        linewidth=2)
ax.plot(np.array(Ns[:]) ** 2, nbit_per_lbfgs_mech_G, "g", marker='o',
        label=r'Mechanical equilibrium - Green ',
        linewidth=2)
ax.semilogx(np.array(Ns) ** 2, nbit_per_lbfgs_adjoint_GJ, "k-.", marker='x',
            label=r'Adjoint problem - Green-Jacobi',
            linewidth=2)
ax.semilogx(np.array(Ns) ** 2, nbit_per_lbfgs_mech_GJ, "k", marker='o',
