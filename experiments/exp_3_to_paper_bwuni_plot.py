...
for w_mult in [4.00]:  # np.arange(0.1, 1., 0.1):# [1]:
    for eta_mult in [0.01]:  # np.arange(0.05, 0.5, 0.05):#[0.1 ]:
        ...
        name = (f'{script_name}_N{N}_Et_{E_target}_Pt_{poison_target}_P0_{poison_0}_w{w_mult}_eta{eta_mult}_p{p}_mpi{cores}_nlc_{nb_load_cases}_e_{energy_objective}')

    if plot_figs:

        phase_field = np.load(os.getcwd() +'/muFFTTO_test/experiments/exp_data/' + name + '.npy', allow_pickle=True, max_header_size=100000)

        xopt = np.load(os.getcwd() +'/muFFTTO_test/experiments/exp_data/' + name + 'xopt_log.npz', allow_pickle=True, max_header_size=100000)
...
if plot_movie:
    for nb_tiles in [1,3]:
        ...
        def update(i):
            ...
            xopt_it = np.load(os.getcwd() +'/muFFTTO_test/experiments/exp_data/' +name+ f'_it{i + 1}.npy', allow_pickle=True)
            ax.clear()
            ax.imshow(np.tile(xopt_it, (nb_tiles, nb_tiles)),  cmap=mpl.cm.Greys, vmin=0, vmax=1)

            ax.set_title('iteration {}, optimizer {}'.format(i, optimizer))
     ...
     # Save as a GIF
        ani.save(os.getcwd() +f'/muFFTTO_test/experiments/figures/movie{nb_tiles}_exp2_imshow_{name}.gif', writer=PillowWriter(fps=40))