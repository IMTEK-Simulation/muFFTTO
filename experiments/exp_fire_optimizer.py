...
from exp_2D_elasticity_TO_multiple_load_cases_Paper import objective_function_multiple_load_cases
...
for w in [10, ]:  # np.arange(0.01, 1.5, 0.05):#
    for eta_mult in [4, ]:  # np.arange(0.01, 0.5, 0.05):#
        ...
        eta = pixel_diameter / eta_mult  # * discretization.pixel_size[0]
        ...
        if __name__ == '__main__':
            ...
            # # material distribution
            # phase_field_0 = microstructure_library.get_geometry(nb_voxels=discretization.nb_of_pixels,
            ...
            f_for_fire = lambda x: objective_function_multiple_load_cases(x)[0]
            df_for_fire = lambda x: objective_function_multiple_load_cases(x)[1]

            xopt_FE_Fire = solvers.optimize_fire(x0=phase_field_0,
                                                 f=f_for_fire,
                                                 df=df_for_fire,
                                                 params=None,
                                                 atol=1e-6,
            ...
            phase_field_sol_FE_MPI = xopt_FE_Fire[0].reshape([1, 1, *discretization.nb_of_pixels])
            sensitivity_sol_FE_MPI = df_for_fire(phase_field_sol_FE_MPI.reshape(-1)).reshape(
                [1, 1, *discretization.nb_of_pixels])

            file_data_name = f'1muFFTTO_{problem_type}_random_init_N{number_of_pixels[0]}_E_target_{E_target}_Poisson_{poison_target}_Poisson0_{poison_0}_w{w}_eta{eta_mult}_p{p}_bounds={bounds}_FE_NuMPI{MPI.COMM_WORLD.size}_nb_load_cases_{nb_load_cases}_energy_objective_{energy_objective}_Fire.npy'  # print('rank' f'{MPI.COMM_WORLD.rank:6} ')
