...
for load_case in np.arange(nb_load_cases):
    ...
    target_energy[load_case] = np.einsum('ij,ijkl,lk->', macro_gradients[load_case], elastic_C_target,
...
for w_mult in [1.0, ]:  # np.arange(0.1, 1., 0.1):#[1, ]:  # np.arange(1, 2, 1):  # [2, ]:  #
    for eta_mult in [0.01, ]:
        ...
        if __name__ == '__main__':
            ...
            # phase_field_0_I = np.random.randint(0, high=2, size=discretization.get_scalar_sized_field().shape) ** 1
            ...
            # phase_field_0 = apply_filter(phase_field_0)
            # phase_field_0[np.logical_and(np.abs(phase_field_0) > 0.49,
            #                              np.abs(phase_field_0) < 0.51)] = 0.49
            # # phase*=np.random.random(discretization.get_scalar_sized_field().shape)
            # phase[0,0,0, 0] = 0
            ...
            file_data_name = (
                f'1muFFTTO_elasticity_random_init_N{64}_E_target_{E_target}_Poisson_{poison_target}_Poisson0_{poison_0}_w{w}_eta{2}_p{p}_bounds=False_FE_NuMPI{6}.npy')

            load_init_from_smaller_grid = False
            if load_init_from_smaller_grid:
                file_data_name = (
                    f'1muFFTTO_elasticity_random_init_N{64}_E_target_{E_target}_Poisson_{poison_target}_Poisson0_{poison_0}_w{w}_eta{1}_p{p}_bounds=False_FE_NuMPI{6}.npy')
                ...
                if MPI.COMM_WORLD.size == 1 or None:
                    ...
                    phase_64 = np.load(f'exp_data/' + file_data_name)
                    phase_128 = sp.ndimage.zoom(phase_64, zoom=2, order=1)
                    phase_field_0[0, 0] = phase_128
                else:

                    discretization_64 = domain.Discretization(cell=my_cell,
                                                              nb_of_pixels_global=(64, 64),
                                                              discretization_type=discretization_type,
                                                              element_type=element_type)
                    phase_64 = load_npy(folder_name + file_data_name,
                                        tuple(discretization_64.fft.subdomain_locations),
                                        tuple(discretization_64.nb_of_pixels_global), MPI.COMM_WORLD)
                    phase_128 = sp.ndimage.zoom(phase_64, zoom=2, order=1)
                    # phase_field_0[0, 0] = phase_128
                    phase_field_0[0, 0] = phase_128[discretization.fft.subdomain_slices]
            ...
            if load_init_from_same_grid:
                ...
                # phase_field_0[0, 0] = phase_128
                #     print(phase.shape)
                #     print(tuple(discretization.fft.subdomain_locations))
                #     print(tuple(discretization.nb_of_pixels_global))
                #
                #
                #
                #     print(discretization.fft.subdomain_slices)
                #     print(phase[discretization.fft.subdomain_slices].shape)
                # print(phase_field_0.shape)
                # print(phase.max())

                phase_field_0[0, 0] = phase  # [discretization.fft.subdomain_slices]

            # my_cell_conductivity = domain.PeriodicUnitCell(domain_size=domain_size,
            #                                               problem_type='conductivity', )
            # discretization_conductivity = domain.Discretization(cell=my_cell_conductivity,
            #                                                     nb_of_pixels_global=number_of_pixels,
            #                                                     discretization_type=discretization_type,
            #                                                     element_type=element_type)
            # conductivity_A_1 = 2*eta*np.array([[1., 0], [0, 1.0]])
            #
            # material_data_field_A_1 = np.einsum('ij,qxy->ijqxy', conductivity_A_1,
            #                                     np.ones(np.array([discretization.nb_quad_points_per_pixel,
            #                                                       *discretization.nb_of_pixels])))
            # preconditioner_laplace = discretization_conductivity.get_preconditioner_NEW(
            #     reference_material_data_field_ijklqxyz=material_data_field_A_1)
            # M_fun_laplace = lambda x: discretization.apply_preconditioner_NEW(
            #     preconditioner_Fourier_fnfnqks=preconditioner_laplace,
            #     nodal_field_fnxyz=x)
            if MPI.COMM_WORLD.size == 1:
                print('rank' f'{MPI.COMM_WORLD.rank:6} phase=' f'{phase_field_0[0, 0]}')
                plt.figure()
            ...
            # file_data_name = f'test2_eta_1muFFTTO_{problem_type}_random_init_N{number_of_pixels[0]}_E_target_{E_target}_Poisson_{poison_target}_Poisson0_{poison_0}_w{w_mult}_eta{eta_mult}_p{p}_bounds={bounds}_FE_NuMPI{MPI.COMM_WORLD.size}_nb_load_cases_{nb_load_cases}_energy_objective_{energy_objective}_random_{random_initial_geometry}.npy'  # print('rank' f'{MPI.COMM_WORLD.rank:6} ')
            #
            # save_npy(folder_name + file_data_name, phase_field_0[0, 0],
            #          tuple(discretization.fft.subdomain_locations),
            #          tuple(discretization.nb_of_pixels_global), MPI.COMM_WORLD)
            ...
            def my_callback(result_norms):
                ...
                file_data_name_it = f'eta_3muFFTTO_{problem_type}_random_init_N{number_of_pixels[0]}_E_target_{E_target}_Poisson_{poison_target}_Poisson0_{poison_0}_w{w_mult}_eta{eta_mult}_p{p}_bounds={bounds}_FE_NuMPI{MPI.COMM_WORLD.size}_nb_load_cases_{nb_load_cases}_energy_objective_{energy_objective}_random_{random_initial_geometry}_it{iteration}'  # print('rank' f'{MPI.COMM_WORLD.rank:6} ')

                folder_name = 'experiments/exp_data/'
                save_npy(folder_name + file_data_name_it + f'.npy', result_norms[6].reshape([*discretization.nb_of_pixels]),
                         tuple(discretization.fft.subdomain_locations),
                         tuple(discretization.nb_of_pixels_global), MPI.COMM_WORLD)
                print(folder_name + file_data_name_it + f'.npy')
                # print("Callback received:", result_norms[0])
            ...
            xopt_FE_MPI['norms_f'] = norms_f
            xopt_FE_MPI['norms_delta_f'] = norms_delta_f
            xopt_FE_MPI['norms_max_grad_f'] = norms_max_grad_f
            xopt_FE_MPI['norms_norm_grad_f'] = norms_norm_grad_f
            xopt_FE_MPI['norms_max_delta_x'] = norms_max_delta_x
            xopt_FE_MPI['norms_norm_delta_x'] = norms_norm_delta_x
            xopt_FE_MPI['norms_sigma'] =norms_sigma
            xopt_FE_MPI['norms_pf'] =norms_pf
            ...
            file_data_name = f'eta_2muFFTTO_{problem_type}_random_init_N{number_of_pixels[0]}_E_target_{E_target}_Poisson_{poison_target}_Poisson0_{poison_0}_w{w_mult}_eta{eta_mult}_p{p}_bounds={bounds}_FE_NuMPI{MPI.COMM_WORLD.size}_nb_load_cases_{nb_load_cases}_energy_objective_{energy_objective}_random_{random_initial_geometry}'  # print('rank' f'{MPI.COMM_WORLD.rank:6} ')

            folder_name = 'experiments/exp_data/'
            save_npy(folder_name + file_data_name + f'.npy', phase_field_sol_FE_MPI[0, 0],
                     tuple(discretization.fft.subdomain_locations),
                     tuple(discretization.nb_of_pixels_global), MPI.COMM_WORLD)
            print(folder_name + file_data_name + f'.npy')

            save_npy(folder_name + f'sensitivity' + file_data_name + f'.npy', sensitivity_sol_FE_MPI[0, 0],
                     tuple(discretization.fft.subdomain_locations),
                     tuple(discretization.nb_of_pixels_global), MPI.COMM_WORLD)
            print(folder_name + file_data_name + f'.npy')
            #  quit()
            of = objective_function_multiple_load_cases(phase_field_sol_FE_MPI)[0]
            if MPI.COMM_WORLD.rank == 0:
            ...
            for load_case in np.arange(nb_load_cases):
                ...
                homogenized_stresses[load_case] = discretization.get_homogenized_stress(
                    ...
                    formulation='small_strain')
                xopt_FE_MPI['target_stress'] = target_stresses[load_case]
                xopt_FE_MPI['homogenized_stresses'] = homogenized_stresses[load_case]
                stress = np.einsum('ijkl,lk->ij', elastic_C_0, macro_gradients[load_case])
                if MPI.COMM_WORLD.rank == 0:
                    print('init_stress for load case {} = \n {}'.format(load_case, stress))
                    print('Target_stress for load case {} = \n {}'.format(load_case, target_stresses[load_case]))
                    print('Optimized stressfor load case {} = \n {}'.format(load_case, homogenized_stresses[load_case]))
                    print('Stress diff for load case {} = \n {}'.format(load_case, target_stresses[load_case] -
                                                                        homogenized_stresses[load_case]))
                    print('Target_stress norm for load case {} = \n {}'.format(load_case,
                                                                               np.sum(target_stresses[load_case] ** 2)))
                    print('Stress  diff norm for load case {} = \n {}'.format(load_case, np.sum(
                        (target_stresses[load_case] - homogenized_stresses[load_case]) ** 2)))
                #
