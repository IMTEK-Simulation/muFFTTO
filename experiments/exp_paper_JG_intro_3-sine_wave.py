...
for kk in np.arange(np.size(nb_pix_multips)):
    ...
    geometry_ID = 'sine_wave_'   # 'square_inclusion'#'circle_inclusion'#random_distribution
    ...
    for i in np.arange(ratios.size):
        ...
        displacement_field, norms = solvers.PCG(K_fun, rhs, x0=None, P=M_fun, steps=int(1000), toler=1e-20, norm_type='rr')
        nb_it[kk - 1, i] = (len(norms['residual_rr']))
        ...
        displacement_field_combi, norms_combi = solvers.PCG(K_fun, rhs, x0=None, P=M_fun_combi, steps=int(1000),
                                                            toler=1e-20, norm_type='rr')
        nb_it_combi[kk - 1, i] = (len(norms_combi['residual_rr']))
        ...
        displacement_field_Jacobi, norms_Jacobi = solvers.PCG(K_fun, rhs, x0=None, P=M_fun_Jacobi, steps=int(1000),
                                                              toler=1e-20, norm_type='rr')
        nb_it_Jacobi[kk - 1, i] = (len(norms_Jacobi['residual_rr']))
...
# Save as a GIF
ani.save(
    f"./figures/convergence_exp3_sine_{number_of_pixels[0]}comparison{ratios[-1]}_RichardsonJacobi{geometry_ID}_circle_inc_to_smooth_semiloplots3.gif",
...
# Save as a GIF
ani.save(
    f"./figures/convergence_gif_{number_of_pixels[0]}comparison{ratios[-1]}_RichardsonJacobi{geometry_ID}_circle_inc_to_smooth_exp3_sine.gif",
    writer=PillowWriter(fps=1))
...
if plot_evolion:
    for nb_tiles in [1, ]:
        ...
        counter += 1
        plt.tight_layout()
    fname = src + 'introduction_exp3_sine_{}{}'.format(number_of_pixels[0], '.pdf')
...
for i in np.array([0, ratios.size //2 , ratios.size - 1]):
    ...
    ax_1.semilogy(np.arange(1,np.size(norm_rr[i])+1),norm_rr[i], label=f'Green - contrast -' + f'1e{ratios[i]}' , color='green', linestyle=linestyles[counter],
                  marker='o',markerfacecolor='white',markevery=4)
    ax_1.semilogy(np.arange(1,np.size(norm_rr_Jacobi[i])+1),norm_rr_Jacobi[i], label=f'Jacobi - contrast -' + f'1e{ratios[i]}', color='blue', linestyle=linestyles[counter],
                  marker='^',markerfacecolor='white',markevery=4)
    ax_1.semilogy(np.arange(1,np.size(norm_rr_combi[i])+1), norm_rr_combi[i], label=f'Jacobi-Green - contrast -' + f'1e{ratios[i]}', color='black', linestyle=linestyles[counter],
                  marker='x',markerfacecolor='white',markevery=1)
    # x_1.plot(ratios, nb_pix_multips[i] * 32, zs=nb_it_Richardson[i], label='Richardson Green', color='green')
    # ax_1.plot(ratios, nb_pix_multips[i] * 32, zs=nb_it_Richardson_combi[i], label='Richardson Green+Jacobi')
    ...
    counter += 1
fname = src + 'introduction_convergence_exp3_sine_{}{}'.format(number_of_pixels[0], '.pdf')
print(('create figure: {}'.format(fname)))
plt.savefig(fname, bbox_inches='tight')
plt.show()
...
if plot_evolion:
    for nb_tiles in [1, ]:
        ...
        # Save as a GIF
        ani.save(
            f"./figures/movie_exp_paper_JG_exp3_sine_{number_of_pixels[0]}comparison{ratios[-1]}_RichardsonJacobi{geometry_ID}_circle_inc_to_smooth_semiloplots3.gif",
