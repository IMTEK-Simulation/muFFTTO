...
############################################################################################3
degree = 2
knot_u = np.array([0.0, 0.0, 0.0, 1., 1., 2.0, 2.0, 2.0])
knot_v = np.array([0.0, 0.0, 0.0, 1., 1., 2.0, 2.0, 2.0])
...
for n_p in range(nb_nodal_points):
    x_coords[:, n_p, ...] = np.copy(rhs.coords)
...
rhs.p = (1 + np.cos(2 * np.pi * x_coords[0] / nb_grid_points) * np.cos(2 * np.pi * x_coords[1] / nb_grid_points)) ** 1
rhs.p -= np.mean(rhs.p)
