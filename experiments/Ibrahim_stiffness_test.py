...
element_type = 'bilinear_rectangle'#'bilinear_rectangle'#'linear_triangles'
...
for kk in np.arange(np.size(nb_pix_multips)):
    ...
    K_0, G_0 = domain.get_bulk_and_shear_modulus(E=1, poison=0.2)#$ 1, 0.5  #
    K_3, G_3 = domain.get_bulk_and_shear_modulus(E=5, poison=0.2)

    elastic_C_1 = domain.get_elastic_material_tensor(dim=discretization.domain_dimension,
                                                     K=K_0,
                                                     mu=G_0,
                                                     kind='linear')
    print('elastic tangent C1= \n {}'.format(domain.compute_Voigt_notation_4order(elastic_C_1)))
    #elastic_C_3 = domain.get_elastic_material_tensor(dim=discretization.domain_dimension,
                                                     # K=K_3,
                                                     # mu=G_3,
                                                     # kind='linear')
    #print('elastic tangent C3 = \n {}'.format(domain.compute_Voigt_notation_4order(elastic_C_3)))
    #print('elastic tangent c1/C3 = \n {}'.format(domain.compute_Voigt_notation_4order(elastic_C_1)/domain.compute_Voigt_notation_4order(elastic_C_3)))
    ...
    refmaterial_data_field_ = np.einsum('ijkl,qxy->ijklqxy', elastic_C_1,
                                           np.ones(np.array([discretization.nb_quad_points_per_pixel,
                                                             *discretization.nb_of_pixels])))
    ...
    phase_field = np.random.random(initial_phase_field.shape)
    ...
    for i in np.arange(ratios.size):
        ...
        # quad_field_shape = list(phase_fnxyz.shape)  # [f,n,x,y,z]
        # quad_field_shape = np.insert(quad_field_shape, 1, discretization.nb_quad_points_per_pixel)
        # phase_field_at_quad_poits_1qnxyz = np.zeros(quad_field_shape)
        ...
        reduced_K[:, 0] = 0
        reduced_K[0, :] = 0
        reduced_K[:, np.prod(number_of_pixels, dtype=int)] = 0
        reduced_K[np.prod(number_of_pixels, dtype=int), :] = 0
        reduced_K[0, 0] = 1
        reduced_K[np.prod(number_of_pixels, dtype=int), np.prod(number_of_pixels, dtype=int)] = 1
        # Remove rows
        #reduced_matrix = np.delete(np.delete(M, [0,1], axis=0), [0,1], axis=1)
        reduced_DB = np.copy(M)
        reduced_DB [:, 0] = 0
        reduced_DB [0,:] = 0
        reduced_DB [:, np.prod(number_of_pixels,dtype=int)] = 0
        reduced_DB [ np.prod(number_of_pixels,dtype=int),:] = 0
        reduced_DB[0, 0] = 1
        reduced_DB[np.prod(number_of_pixels,dtype=int), np.prod(number_of_pixels,dtype=int)] = 1
        #eig = sc.linalg.eigh(a=K, b=None, eigvals_only=True)
        #eigDB= sc.linalg.eigh(a=reduced_DB, b=None, eigvals_only=True)
        eig = sc.linalg.eigh(a=reduced_DB, b=None, eigvals_only=True)
        plt.figure()
        # plt.plot(eig)
        plt.plot(np.sqrt(eig) )

        plt.title('M eig')
        plt.show()
        #hist, _ = np.histogram(eig, density=True)
        DOS, bins = np.histogram(np.sqrt(eig), bins=50, density=True)
        x_omega = (bins[1:] - bins[:-1]) / 2 + bins[:-1]
        plt.figure()
       # plt.plot(eig)
        plt.plot(x_omega,DOS )

        plt.title('M x_omega /DOS ')
        plt.show()

        eig = sc.linalg.eigh(a=reduced_K, b=None, eigvals_only=True)
        plt.figure()
        # plt.plot(eig)
        plt.plot(np.sqrt(eig))
       ...
       # kontrast_2.append(eig[-3] / eig[np.argmax(eig > 0)])
        ...
        print(f'min={sorted(eig)[2]:.4g}, max={sorted(eig)[-3]:.4g}')
        kontrast_2.append((max_J / min_J))
        ax2.loglog(sorted(eig)[2:-3], label=f'{i}',marker='.', linewidth=0, markersize=1)
...
for i in np.arange(ratios.size,step=1):
    ...
    ax_1.semilogy(convergence_Green, '--',label='estim', color='r')
    ax_1.semilogy(convergence_Green_Jacobi, '--',label='estim', color='b')
...
# Save as a GIF
ani.save(f"./figures/converg213ence__es2tgif_{number_of_pixels[0]}comparison{ratios[-1]}_RichardsonJacobi{geometry_ID}_circle_inc_to_smooth_semiloplots3.gif",
         writer=PillowWriter(fps=1))

plt.show()
quit()
#-------------------------------------------------------------------------------------------------------
# for i in np.arange(ratios
# .size,step=1):
#     kappa=kontrast[i]
