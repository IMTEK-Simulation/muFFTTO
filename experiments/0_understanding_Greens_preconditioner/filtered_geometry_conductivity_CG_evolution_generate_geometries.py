...
parser.add_argument("-g", "--geometry", type=str,
                    choices=["circle_inclusion", "cos_wave"],
                    default="cos_wave")
parser.add_argument("-n", "--nb_pixel", type=int, default=None,
                    help="Single grid size. If omitted, runs all sizes 8..128.")
args = parser.parse_args()

script_name = 'filtered_geometry_conductivity_CG_evolution'
...
domain_size = [1, 1]
pixel_sizes = [args.nb_pixel] if args.nb_pixel is not None else [2 ** (i + 1) for i in range(2, 10)]
for nb_pix in pixel_sizes:
