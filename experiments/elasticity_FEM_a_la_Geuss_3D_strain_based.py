...
K_fun_I = lambda x: D_t(
    ddot42(mat_data_ijklqxy,
           D(u_ixy=x.reshape(displacement_shape)))).reshape(-1)
# right hand side vector
b_disp_I = -D_t(ddot42(A=mat_data_ijklqxy, B=E_ijqxy)).reshape(-1)  # right-hand side
...
for d in range(n_u_dofs):
    ...
    M_diag_ijxy[:, d, ...] = D_t(ddot42(A=ref_mat_data_ijkl, B=D(u_ixy=unit_impuls_ixy)))
...
gamma_0 = lambda x: D(
    M_fun_I(D_t(ddot42(mat_data_ijklqxy, x.reshape(grad_shape)))).reshape(displacement_shape).real).reshape(-1)
b_strain_I = -gamma_0(E_ijqxy)
