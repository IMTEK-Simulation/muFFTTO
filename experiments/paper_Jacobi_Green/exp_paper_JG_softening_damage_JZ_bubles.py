...
parser = argparse.ArgumentParser(
    prog="exp_paper_JG_softening_damage_JZ_bubles.py",
    description="Solve non-linear elasticity with exponential softening damage."
)
...
def softening_damage_q_points(strain_ijqxyz,
    ...
    # Term B: - (C_linear : eps) \otimes d(damage)/d(eps)
    # d(damage)/d(eps_eq)
    dd_deq = (dmax / eps0) * np.exp(-strain_eq_qx / eps0)
    
    # deq/deps = (2/3) * eps_dev / eps_eq
    deq_deps = (2./3.) * strain_dev_ijqxyz_s / strain_eq_safe
    
    term_B = - dd_deq * np.einsum('ij...,kl...->ijkl...', total_lin_stress, deq_deps)
    
    # Symmterize term_B to ensure tangent symmetry
    term_B = 0.5 * (term_B + np.einsum('ijkl...->klij...', term_B))
    
    # Ensure positive definiteness by damping term_B if necessary
    # or just use Picard (term_A) if needed. 
    # For now, let's try to just use term_A (Picard iteration) to see if it converges
    # tangent_ijklqxyz.s[..., phase_xyz] = term_A + term_B
    # damping = 0.5
    # tangent_ijklqxyz.s[..., phase_xyz] = term_A + damping * term_B
...
def plot_results(iteration, total_strain, stress, tangent, discretization):
    ...
    # 3. Damage Field
    # damage = dmax * (1.0 - np.exp(-strain_eq / eps0))
    ...
    plot_dir = file_folder_path + '/figures/' + script_name + '/'
...
if save_results:
    ...
    save_npy(fn=data_folder_path + results_name + f'.npy',
             ...
             subdomain_locations=tuple(discretization.fft.subdomain_locations),
...
for inc in range(ninc):
    ...
    while True:
        ...
        # eps_eq calculation (similar to plot_results)
