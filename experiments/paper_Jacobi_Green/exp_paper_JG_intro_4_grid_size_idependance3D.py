...
domain_size = [1, 1, 1]
nb_pix_multips = [ 2,3,4,5,6 ]  # ,2,3,3,2,
...
for nb_starting_phases in np.arange(np.size(nb_pix_multips)):
    ...
    for kk in np.arange(np.size(nb_pix_multips[nb_starting_phases:])):
        ...
        # system set up
        number_of_pixels = ( 2**nb_pix_multip,2**nb_pix_multip,2**nb_pix_multip)
        ...
        for i in np.arange(ratios.size):
            ...
            #phase_field_smooth = sc.ndimage.zoom(phase_fied_small_grid, zoom=nb_pix_multip, order=0)
                ...
                phase_field_smooth = np.repeat(phase_field_smooth, 2 ** (kk), axis=2)
           ...
           # phase_field_at_quad_poits_1qnxyz = discretization.get_displacement_gradient_size_field()
            #phase_field_at_quad_poits_1qnxyz[0, :, 0, ...] = phase_field
            ...
            displacement_field, norms = solvers.PCG(K_fun, rhs, x0=None, P=M_fun, steps=int(1000), toler=1e-6, norm_type='rr')
