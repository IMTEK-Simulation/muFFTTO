...
def generate_smaller_grids(geometry_name='bubbles_',N=256):
    # load fines geometry -- generated jsut finnest, and the coarsen others


    results_name = (geometry_name+ f'dof={N}')
...
if generate_cube:
    ...
    for nb_pixels_power in pixel_sizes:
       ...
       # nb_laminates = 2 ** nb_pixels_power
        ...
        def generate_cube(coords, num_inclusions, radius, seed=None):
            """
        ...
        results_name = (f'cube_' + f'dof={nb_pixels_power}')
        to_save = np.copy(geometry)
...
if generate_prism:
    ...
    for nb_pixels_power in pixel_sizes:
       ...
       # nb_laminates = 2 ** nb_pixels_power
        ...
        def generate_prism(coords, num_inclusions, radius, seed=None):
            """
            Generate a boolean array with randomly distributed spherical inclusions
            with periodic boundary conditions.
            ...
            # Initialize output array
            inclusions = np.zeros(coords.shape[1:], dtype=np.int32)
            inclusions[:Nx // 2, :Ny // 2, :] = 1

            return inclusions


        geometry = generate_prism(
            discretization.fft.coords,
            num_inclusions=1,
            radius=0.2,
            seed=42
        ...
        results_name = (f'prism_' + f'dof={nb_pixels_power}')
        to_save = np.copy(geometry)
        np.save(data_folder_path + results_name + f'.npy', to_save)

        generate_smaller_grids(geometry_name='prism_', N=256)
...
if generate_spheres:
    ...
    pixel_sizes =np.array([16,32,50,64,100,128,150,200,256])
for nb_pixels_power in pixel_sizes:
