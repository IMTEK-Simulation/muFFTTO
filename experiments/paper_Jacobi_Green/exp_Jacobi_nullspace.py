...
if compute:
    ...
    nb_it = np.zeros((3, 2))
    nb_it_combi = np.zeros((3, 2))
    nb_it_Jacobi = np.zeros((3, 2))
    nb_it_Richardson = np.zeros((3, 2))
    nb_it_Richardson_combi = np.zeros((3, 2))
    ...
    for sharp in [False, True]:
        ...
        if sharp:
            ...
            phase_field[phase_field < 0.5] = phase_field_min  # phase_field_min#
        ...
        preconditioner_Green = discretization.get_preconditioner_Green_fast(reference_material_data_ijkl=elastic_C_1)
        ...
        if save_results:
            ...
            results_name = (f'strain_fluc_field_' + f'_sharp{sharp}')
            ...
            np.save(data_folder_path + results_name + f'_G.npy', strain_fluc_field.s)
            results_name = (f'energy_evols_G_' + f'_sharp{sharp}')
            np.save(data_folder_path + results_name + f'_G.npy', energy_evols_G)

        nb_it[0, counter] = (len(norms['residual_rr']))
        ...
        if save_results:
            results_name = (f'displacement_field_' + f'_sharp{sharp}')
            ...
            results_name = (f'energy_evols_GJ_' + f'_sharp{sharp}')
            np.save(data_folder_path + results_name + f'_GJ.npy', energy_evols_GJ)


        nb_it_combi[0, counter] = (len(norms_combi['residual_rr']))
        ...
        results_name = f'N64_sharp_{sharp}'
