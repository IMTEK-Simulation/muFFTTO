    ...
    for nb_starting_phases in np.arange(np.size(geom_n)):
        ...
        for nb_discretization_index in np.arange(np.size(geom_n[nb_starting_phases:])):
            ...
            for i in np.arange(ratios.size):
                ...
                nb_it_combi[nb_discretization_index + nb_starting_phases, nb_starting_phases, i] = (len(norms_combi['residual_rr']))
               ...
               # norm_energy_lb_combi.append(norms_combi['energy_lb'])
                ...
                nb_it_Jacobi[nb_discretization_index + nb_starting_phases, nb_starting_phases, i] = (len(norms_Jacobi['residual_rr']))
                ...
                _info['nb_of_sampling_points'] = discretization.nb_of_pixels_global
                # phase_field_sol_FE_MPI = xopt.x.reshape([1, 1, *discretization.nb_of_pixels])
                _info['norm_rMr_G'] = norms
                _info['norm_rMr_J'] = norms_Jacobi
                _info['norm_rMr_JG'] = norms_combi
                script_name='exp_paper_JG_linear_conv_2_no_eigens'
                file_data_name = (
                    f'{script_name}_T{nb_discretization_index}_G{nb_starting_phases}.npy')
