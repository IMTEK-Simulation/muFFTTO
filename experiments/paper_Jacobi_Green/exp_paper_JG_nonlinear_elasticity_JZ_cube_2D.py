...
parser = argparse.ArgumentParser(
    prog="exp_paper_JG_nonlinear_elasticity_JZ_cube_2D.py",
...
parser.add_argument("-exp", "--exponent_elastic", default="2")
parser.add_argument(
...
geom_folder_path = file_folder_path + '/exp_data/' + 'exp_paper_JG_nonlinear_elasticity_JZ_generate_geom_2D/'
...
def nonlinear_elastic_q_points(strain_ijqxyz,
    ...
    n = kwargs['n']  # 5.0  # 3.0  # hardening exponent  # # 5.0
    # 10.0
    # volumetric strain
    strain_trace_qx = np.einsum('ii...', strain_ijqxyz.s[..., phase_xyz]) / 2
    # deviatoric strain
    ...
    # equivalent strain
    strain_eq_qx = discretization.get_quad_field_scalar(name='strain_eq_qx')
    strain_dev_ddot = np.einsum('ijqx...,jiqx...-> qx...', strain_dev_ijqxyz_s,
                                strain_dev_ijqxyz_s)
    strain_eq_qx.s[..., phase_xyz] = np.sqrt((2. / 3.) * strain_dev_ddot)

    # numerical stability
    threshold = 1e-15
    mask = strain_eq_qx.s[..., phase_xyz] > threshold
    strain_eq_safe = np.where(mask, strain_eq_qx.s[..., phase_xyz], 1.0)  # Maybe zero?

    # Stress calculation
    # volumetric part
    stress_phase = np.zeros_like(strain_ijqxyz.s[..., phase_xyz])
    for d in np.arange(discretization.domain_dimension):
        stress_phase[d, d] =  2* K * strain_trace_qx
...
def constitutive(strain_ijqxyz,
                 ...
                 K4_ijklqxyz,
                 build_tangent=True):
    strain_eq_qx = constitutive_q_points(strain_ijqxyz=strain_ijqxyz,
...
strain_eq_qx = constitutive(total_strain_field, stress_field, K4_ijklqyz)
...
if save_results:
    ...
    save_npy(fn=data_folder_path + results_name + f'.npy',
             data=K4_ijklqyz.s[0, 1, 0, 1].mean(axis=0),
...
macro_gradient_inc = np.zeros(shape=(discretization.domain_dimension, discretization.domain_dimension))
...
for inc in range(ninc):
    ...
    if save_results:
        temp_max_size_ = {'nb_max_subdomain_grid_pts': discretization.nb_max_subdomain_grid_pts}
        results_name = (f'displacement_fluctuation_field' + f'_exp_{n_exp}_it{iteration_total}')
        save_npy(data_folder_path + results_name + f'.npy', displacement_fluctuation_field.s.mean(axis=1),
                 tuple(discretization.subdomain_locations_no_buffers),
                 tuple(discretization.nb_of_pixels_global), MPI.COMM_WORLD)
        results_name = (f'strain_eq' + f'_exp_{n_exp}_it{iteration_total}')
        save_npy(data_folder_path + results_name + f'.npy', strain_eq_qx.s[0, 0].mean(axis=0),
                 tuple(discretization.subdomain_locations_no_buffers),
                 tuple(discretization.nb_of_pixels_global), MPI.COMM_WORLD)
        # save K4_ijklqyz
    ...
    while True:
        ...
        if preconditioner_type == 'Green':  # or iteration_total == 0 or iiter == 0:
            ...
            def M_fun_Jacobi(x, Px):
                ...
                # np.mean(Px.s,axis=(-1,-2,-3))
                # Px.s -=discretization.mpi_reduction.mean(Px.s,axis=(-1,-2,-3), keepdims=True)
