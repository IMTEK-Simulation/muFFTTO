...
ratios = np.arange(0., 1.1, 0.2)
ratios = np.arange(2,17)  # 65 17  33
...
#        phase_field = scale_field(phase_field, min_val=np.power(10, 1), max_val=10 ** 2)
        phase_field = scale_field(phase_field, min_val=10 ** 2 -2, max_val=10 ** 2)
        ...
        file_data_name = (
            f'{script_name}_gID{geometry_ID}_T{nb_pix_multip}_G{ratio}_kappa{ratio}.npy')
