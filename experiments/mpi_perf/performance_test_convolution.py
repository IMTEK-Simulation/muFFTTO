...
def memory_loop(phase_field_1nxyz):
    ...
    weights = np.ones(discretization.quadrature_weights.shape)

    #   convolution operator
    discretization.fft.communicate_ghosts(field=phase_field_at_quad_poits_1qxyz)
    # apply B^transposed via the convolution operator
    conv_op.transpose(quadrature_point_field=phase_field_at_quad_poits_1qxyz,
                      nodal_field=phase_field_1nxyz,
                      weights=weights)
...
if __name__ == '__main__':
    ...
    script_name = os.path.splitext(os.path.basename(__file__))[
                      0] + f'_ranks_{MPI.COMM_WORLD.Get_size()}' + f'_N_{number_of_pixels[0]}'
    ...
    if MPI.COMM_WORLD.rank == 0:
        print(f'_ranks_{MPI.COMM_WORLD.Get_size()}' + f'_N_{number_of_pixels[0]}')
