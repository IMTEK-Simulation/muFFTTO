...
import matplotlib.pyplot as plt
from conductivity_CG_analysis_base import matrix_sqrt_eig
...
for geometry_ID_index, geometry_ID in enumerate(['linear', 'right_cluster_x3',
                                                 'left_cluster_x3']):  # ,'sine_wave_','linear', 'right_cluster_x3', 'left_cluster_x3'
    ...
    for nb_starting_phases in np.arange(np.size(nb_pix_multips)):
        ...
        for kk in np.arange(np.size(nb_pix_multips[nb_starting_phases:])):
            ...
            for i in np.arange(ratios.size):
                ...
                material_data_field_C_0.s[...] = elastic_C_1[..., np.newaxis, np.newaxis, np.newaxis] * phase_field[
                    np.newaxis, ...]
                ...
                displacement_field, norms = solvers.PCG(K_fun_G, rhs_G, x0=M_sym @ x0.s.flatten(),
                                                        P=M_null,
                ...
                norm_energy_lb.append(norms['energy_iter_error'])
                # print(nb_it)
                #########
                ...
                gs = fig.add_gridspec(3, 1, hspace=1.0, wspace=0.4, width_ratios=[1],
                                      height_ratios=[1, 1, 1])
                ...
                if ratio == 0:
                    ...
                    # ax_0.set_yticks([1 / np.power(10, ratio), 0.5, 1])
                    # ax_0.set_yticklabels([f'$10^{{{-ratio}}}$', 0.5, 1])
                    ax_0.set_ylim([1, np.power(10, ratio)])
                    ax_0.set_yticks([1, np.power(10, ratio)])
                    ax_0.set_yticklabels([1, f'$10^{{{ratio}}}$'])
                ax_0.set_xlim([0, 1])
                # ax_0.text(-0.25, 1.1, '(a)', transform=ax_0.transAxes)
                ...
                ax_1.set_title(f'nb phases {2 ** (nb_starting_phases)}, nb pixels {number_of_pixels[0]}', wrap=True)
                ax_1.semilogy(np.arange(1, len(norm_energy_lb[-1]) + 1), norm_energy_lb[-1] / norm_energy_lb[-1][0],
                              label=r'$||r_{k}||_{G^{-1}} $  --- Green', color=colors[geometry_ID_index],
                              linestyle='--')
                # ax_1.text(-0.2, 1.05, '(c)', transform=ax_1.transAxes)
                ax_1.set_xlabel('PCG iteration - k')
                ax_1.set_ylabel('Norm of residua')
                ...
                # plt.legend(loc='upper right')
                ax_1.set_ylim([1e-8, 1e1])  # norm_rz[i][0]]/lb)
                print(max(map(len, norm_rr)))
                ax_1.set_xlim([1, 60])
                #
                ...
                bins = 100
                # Create the histogram
                sizes = np.asarray(w_i[2:], dtype=float).reshape(-1)
                print(f'shape w_i[2:]= {w_i[2:].shape}')
                print(f'shape np.asarray(sorted(eig_G)[2:])= {np.asarray(sorted(eig_G)[2:]).shape}')
                print(f'shape  np.ones_like(sorted(eig_G)[2:])= { np.ones_like(sorted(eig_G)[2:]).shape}')
                ax_3.scatter(np.asarray(sorted(eig_G)[2:]), np.ones_like(sorted(eig_G)[2:]), color=colors[geometry_ID_index], s=sizes)
                # ax_3.hist(sorted(eig_G)[2:], bins=bins, color='Green', label=f'Green', edgecolor='black',
                #           alpha=0.99)  # , marker='.', linewidth=0, markersize=5)
                # ax_3.hist(sorted(eig_JG)[2:], bins=bins, color='b', label=f'Jacobi-Green', edgecolor='black',
                #           alpha=0.2)
                ...
                if ratio == 0:
                    ...
                    # ax_3.set_xticklabels([f'$10^{{{-4}}}$', 0.5, 1])
                    ax_3.set_xlim([1, 1e4])
                    ax_3.set_xticks([1, 1e4])
                    ax_3.set_xticklabels([1, f'$10^{{{4}}}$'])
                # ax_3.set_yscale('log')
                #   ax_3.set_xscale('log')
                # ax_3.set_xlim([0, 1])
                ax_3.set_yticks([])
                ax_3.set_title(r'Eigenvalues')

                # plt.legend()
                fname = src + 'JG_cluster_dist_eigenvals_geom_{}_rho{}{}'.format(geometry_ID, ratio,
                                                                                 '.pdf')
                print(('create figure: {}'.format(fname)))
                plt.savefig(fname, bbox_inches='tight')
                plt.show()
