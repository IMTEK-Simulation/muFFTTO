...
domain_size = [1, 1]
nb_pix_multips = [2,3 ]  # ,3,2,[2,3,4,5,6,7,8]
...
for kk in np.arange(np.size(nb_pix_multips)):
    ...
    initial_phase_field.s[initial_phase_field.s[...]<0.5] = 0
    initial_phase_field.s[initial_phase_field.s[...] >= 0.5] = 1
    ...
    for i in np.arange(0,ratios.size):
        ...
        def apply_smoother_log10(phase):
            ...
            # Apply convolution for smoothing
            smoothed_arr = sc.signal.convolve2d(np.log10(phase[0,0]), kernel, mode='same', boundary='wrap')
            smoothed_arr[number_of_pixels[0]//2-1:number_of_pixels[0]//2+1,number_of_pixels[0]//2-1:number_of_pixels[0]//2+1]=-4
        ...
        if i == 0:
            phase_field.s[...] = initial_phase_field.s[...]  + 1e-4
        if i > 0:
            phase_field.s[...] = apply_smoother_log10(initial_phase_field.s[...])
        ...
        material_data_field_C_0.s[...] = conductivity_C_0[...,np.newaxis,np.newaxis,np.newaxis]*phase_field.s[np.newaxis,...]

       # material_data_field_C_0_rho =conductivity_C_0[] *phase_field
       ...
       # reduced_K = np.copy(K)
        ...
        min_val = np.min(phase_field.s[...])
        max_val = np.max(phase_field.s[...])
       ...
       #macro_gradient_field = discretization.get_macro_gradient_field(macro_gradient)
        ...
        preconditioner = discretization.get_preconditioner_Green_mugrid(reference_material_data_ijkl=refmaterial_data_field_)
        ...
        norms_cg = dict()
        norms_cg['residual_rr'] = []
        norms_cg['residual_rz'] = []
        ...
        def callback(it, x, r, p, z, stop_crit_norm):
            ...
            norms_cg['residual_rr'].append(norm_of_rr)
            norms_cg['residual_rz'].append(norm_of_rz)
        ...
        norms=[]
        nb_it[kk - 1, i] = (len(norms_cg['residual_rz']))
