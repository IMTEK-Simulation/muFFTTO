...
def run_simple_CG_Green(initial, RHS, kappa):
    ...
    for geometry_ID in [
        ...
        for ii in np.arange(np.size(geom_n)):  # loop over number of laminate layers
            G = geom_n[ii]
            nb_starting_phases = 2 ** G
            print(f'G = {G}')
            j2 = [x for x in discretization_n if x >= G]
            counter = 0
            for T in j2:  # loop over number of discretization points
                ...
                for i in np.arange(ratios.size):
                    ratio = ratios[i]
                    nb_laminates = nb_starting_phases
                    if counter == 0:
                    ...
                    if counter > 0:
                        ...
                        phase_field_smooth = np.repeat(phase_fied_small_grid, 2 ** (counter), axis=0)
                        phase_field_smooth = np.repeat(phase_field_smooth, 2 ** (counter), axis=1)

                    phase_field_ = np.abs(phase_field_smooth)
                    ...
                    results_name = f'T{discretization.nb_of_pixels_global[0]}_G{nb_laminates}_kappa{ratio}'
    ...
    ax_global.plot(np.arange(1, size_of_vector), eigens_to_plots[::-1], color='Green', label=f'Green',
    ...
    ax_global.scatter(np.arange(1, size_of_vector), eigens_to_plots[::-1], color='Blue', label=f'Green',
                      alpha=opacities)
    ...
    custom_symbol = [
        ...
        mpl.lines.Line2D([], [], color='Blue', marker='.', linestyle='None',
                         markersize=10, label='Non-zero weights'),
        mpl.lines.Line2D([], [], color='Red', marker='.', linestyle='None',
                         markersize=10, label='Non-zero element of vector')]
    ...
    if plot_eigenvectors:
        ...
        for upper_ax in np.arange(6):
            ...
            ax1.pcolormesh(X_, Y_, np.transpose(phase_field.s[0, 0]),
