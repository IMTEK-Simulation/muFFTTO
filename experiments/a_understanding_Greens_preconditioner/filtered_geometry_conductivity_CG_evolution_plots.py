...
script_name_save = 'filtered_geometry_conductivity_CG_evolution'
...
plot_v3 = False
plot_N32 = False
plot_h_convergence=True
if plot_h_convergence:
    geometry_ID = 'circle_inclusion'  # circle_inclusion", "cos_wave"

    contrast = 1e-4
    #nb_of_filters = 10
    filter_index=1
    #    rhos = [0, 600, 3800, nb_of_filters - 1]
    #rhos = [0, 3, 7, nb_of_filters - 1]
   ...
   # filter_ids = np.arange(nb_of_filters)
    ...
    number_of_pixels=[16, 32, 64, 128, 256,512,1024]#, 512, 1024
    for filter_index in range(2):
        ...
        grad_max_inf = []
        homo_flux = []
        for i, nb_pix in enumerate( number_of_pixels):
            print(nb_pix)

            file_data_name = f'N{nb_pix}_F{filter_index}_kappa{contrast}_ge_{geometry_ID}'


            _info = np.load(data_folder_path + file_data_name + f'_log.npz', allow_pickle=True)
            print(data_folder_path + file_data_name + f'_log.npz')

            geom_folder_path = data_folder_path
            geometry_name = f'phase_field_N{nb_pix}_F{filter_index}_kappa{contrast}_ge_{geometry_ID}'
            ...
            nb_it_G.append(len(_info['norms_G_rr']))
            homo_flux.append(_info['homogenized_A_ij'])
            max_phase_contrast.append(phase_field.max() / phase_field.min())
            fig = plt.figure()
            gs = fig.add_gridspec(2, 2)
            ax1 = fig.add_subplot(gs[0, 0])
            # ax1.imshow(phase_field.s[0, 0], cmap='gray')
            ax1.pcolormesh(np.transpose(phase_field ), cmap=mpl.cm.Greys, vmin=contrast, vmax=1,
                           linewidth=0,
                           rasterized=True)
            ax1.set_aspect('equal')
            ax_cross = fig.add_subplot(gs[0, 1])
            ax_cross.semilogy(phase_field[ :, nb_pix // 2], linewidth=1,
            ...
            ax_cross.set_title(f'filter_index={filter_index} contrast={contrast}', wrap=True)

        plt.plot(number_of_pixels,nb_it_G, 'r', linestyle='-', markevery=1,)

   # plt.loglog(np.pow(number_of_pixels,2), np.asarray(abs((homo_flux-homo_flux[-1])/homo_flux[-1])), 'r', linestyle='-', markevery=1,)
    plt.show()
    print(np.asarray(homo_flux))
...
#    rhos = [0, 600, 3800, nb_of_filters - 1]
    ...
    for filter_index in filter_ids:
        file_data_name = f'N{number_of_pixels[0]}_F{filter_index}_kappa{contrast}_ge_{geometry_ID}'
        ...
        geom_folder_path = data_folder_path
        geometry_name = f'phase_field_N{number_of_pixels[0]}_F{filter_index}_kappa{contrast}_ge_{geometry_ID}'
        phase_field = np.load(geom_folder_path + geometry_name + f'.npy', allow_pickle=True)
 ...
 #   nb_it_GJ = np.asarray(nb_it_GJ)
  #  grad_norm = np.asarray(grad_norm)
   # grad_max = np.asarray(grad_max)
    ...
    ax_global.set_xlim([0, nb_of_filters])
    ...
    ax_global.annotate(text=f'Green',  # \n contrast = 100
                       xy=(filter_ids[5], nb_it_G[:nb_of_filters][5]),
                       xytext=(5., 70.),
    ...
    for geom_ax in np.arange(3):
        ...
        geom_folder_path = data_folder_path# file_folder_path + '/exp_data/' + 'exp_paper_JG_intro_circle/'
        geometry_name =  f'phase_field_N{number_of_pixels[0]}_F{filer_id}_kappa{contrast}_ge_{geometry_ID}'
        phase_field = np.load(geom_folder_path + geometry_name + f'.npy', allow_pickle=True)
    ...
    fig.tight_layout()
    fname = script_name_save + f'_N{number_of_pixels[0]}_v3_ge_{geometry_ID}' + '{}'.format('.pdf')
...
if plot_v3:
    ...
    ax_global.set_ylabel(r'\# PCG iterations')
    ax_global.annotate(text=f'Green',  # \n contrast = 100
                       xy=(filter_ids[4], nb_it[0][:nb_of_filters][4]),
                       xytext=(4., 4.),
                       arrowprops=dict(arrowstyle='->',
                                       color='Green',
                                       lw=1,
