...
def plot_eigenvectors(eigenvectors_1, eigenvectors_2, grid_shape, dim=2, eigenvals=None):
    ...
    divnorm2 = mpl.colors.TwoSlopeNorm(vmin=np.min(np.real(eigenvectors_2)), vcenter=0,
                                       vmax=np.max(np.real(eigenvectors_2)))
...
def plot_rhs(rhs, grid_shape, dim=2):
    ...
    eigenvector_x = np.vstack(
        (np.hstack((eigenvector_x, np.zeros((eigenvector_x.shape[0], 1)))), np.zeros((1, eigenvector_x.shape[1] + 1))))
    eigenvector_y = np.vstack(
        (np.hstack((eigenvector_y, np.zeros((eigenvector_y.shape[0], 1)))), np.zeros((1, eigenvector_y.shape[1] + 1))))
    ...
    circles_sizes[-1, :] = 0
    circles_sizes[:, -1] = 0
    ax_eig_vecs_1.scatter(x, y, s=circles_sizes.flatten(), c='white', cmap='Reds', alpha=1.0, norm=divnorm,
                          edgecolors='black', linewidths=0.1),
    ax_eig_vecs_1.scatter(x, y, s=sizes * 20, c=facecolors, cmap='Reds', alpha=1.0, norm=divnorm, edgecolors='black',
                          linewidths=0),
...
def plot_eigenvector_filling(vectors, grid_shape, dim=2):
    ...
    for k in np.arange(0, len(vectors)):
        ...
        eigenvector_x = np.vstack((np.hstack((eigenvector_x, np.zeros((eigenvector_x.shape[0], 1)))),
                                   np.zeros((1, eigenvector_x.shape[1] + 1))))
        eigenvector_y = np.vstack((np.hstack((eigenvector_y, np.zeros((eigenvector_y.shape[0], 1)))),
                                   np.zeros((1, eigenvector_y.shape[1] + 1))))
        ...
        ax_eig_vecs_1.scatter(x, y, s=sizes * 20, c=facecolors, cmap='Reds', alpha=1.0, norm=divnorm,
...
def run_simple_CG(initial, RHS, kappa):
    ...
    ax_iterations.semilogy(iterations, convergence_4 / 2, "green", linestyle='-.', label=r'  $\kappa=10^4$',
                           linewidth=1)
